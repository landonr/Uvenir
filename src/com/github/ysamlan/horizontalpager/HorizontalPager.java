 ...
 * @version 0.1.0
 */
public final class HorizontalPager extends LinearLayout {
	/*
	...
	/**
	 * Sets up the stroller and touch/fling sensitivity parameters for the
	 * pager.
	 */
	private void init() {
	...
	protected void onLayout(final boolean changed, final int l, final int t,
		...
		for (int i = 0; i < count; i++) {
			Log.v("derp", "childleft: " + childLeft);
			final View child = getChildAt(i);
			if (child.getVisibility() != View.GONE) {
				final int childWidth = (int) (child.getMeasuredWidth());
				child.layout(childLeft, 150, childLeft + childWidth,
						child.getMeasuredHeight());
				childLeft += childWidth+viewSpacing;
			}
		}
	}
		...
		if (duration < 0) {
			...
			mScroller
					.startScroll(
							...
							delta,
							0,
							(int) (Math.abs(delta) / (float) getWidth()/2 * ANIMATION_SCREEN_SET_DURATION_MILLIS));
		} else {
